...
def event_relation(tableOfEvents, numberOfNodes):
    ...
    for node in range(0, numberOfNodes):
        if (node != 0):
            tableOfEvents[node][1] = int(
                input("[DATA] Enter the preceding node index (from 0->%d) for %d node: " % (numberOfNodes - 1, node)))
            if (tableOfEvents[node][1] > numberOfNodes):
                tableOfEvents[node][1] = int(input(
                    "[DATA] Enter a valid preceding node index (from 0->%d) for %d node: " % (numberOfNodes - 1, node)))
...
def rel_events(tableOfEvents, numberOfNodes):
    for node in range(1, numberOfNodes):
        tableOfEvents[node][2] = input(
            "[DATA] Enter the relative time for %d and %d event to take place:" % (node, tableOfEvents[node][1]))
...
def graph_builder(tableOfEvents, processGraph, numberOfNodes):
    ...
    for node in range(1, len(tableOfEvents)):
        processGraph.edge(str(int(tableOfEvents[node][1])), str(int(node)),
                          label=("Forward Pass Time: " + str(int(tableOfEvents[node][2]))))
